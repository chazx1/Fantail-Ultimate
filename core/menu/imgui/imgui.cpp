...
template <typename t>
inline t nigga(t number, t min, t max) {
	if (number < min) {
		return min;
	}
	else if (number > max) {
...
void ImGui::NewFrame() {
	...
	if (g.HoveredWindow && g.IO.MouseWheel != 0.0f && !g.HoveredWindow->Collapsed) {
		...
		if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
			...
			SetWindowScrollY(window, window->Scroll.y - (g.IO.MouseWheel * 0.15f) * window->CalcFontSize() * scroll_lines);
		}
	}
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash) {
	...
	if (text_len > 0) {
		window->DrawList->AddText(g.Font, g.FontSize, ImVec2(pos.x + 1, pos.y + 1), GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end);
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect) {
	...
	if (need_clipping) {
		...
		window->DrawList->AddText(g.Font, g.FontSize, ImVec2(pos.x + 1, pos.y + 1), GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end, 0.0f, &fine_clip_rect);
		window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
	}
	else {
		window->DrawList->AddText(g.Font, g.FontSize, ImVec2(pos.x + 1, pos.y + 1), GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end, 0.0f, NULL);
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags) {
	...
	if (first_begin_of_the_frame) {
		...
		window->WindowPadding = ((flags & ImGuiWindowFlags_ChildWindow) && !(flags & (ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_ComboBox | ImGuiWindowFlags_Popup))) ? style.WindowPadding : style.WindowPadding;
		...
		if (window->Collapsed) {
			...
			// Scrollbars
			window->ScrollbarY = (flags & ImGuiWindowFlags_AlwaysVerticalScrollbar) || ((window->SizeContents.y > window->Size.y) && !(flags & ImGuiWindowFlags_NoScrollbar));
			...
			if (flags & ImGuiWindowFlags_MenuBar) {
				...
				if (flags & ImGuiWindowFlags_ShowBorders)
					window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32(ImVec4(0, 0, 0, 1)));
	...
	ImRect clip_rect;
	clip_rect.Min.x = ImFloor(0.5f + title_bar_rect.Min.x + ImMax(border_size, ImFloor(window->WindowPadding.x *0.1f)));
	clip_rect.Min.y = ImFloor(0.5f + title_bar_rect.Max.y + window->MenuBarHeight() + border_size);
	clip_rect.Max.x = ImFloor(0.5f + window->Pos.x + window->Size.x - window->ScrollbarSizes.x - ImMax(border_size, ImFloor(window->WindowPadding.x  *0.1f)));
...
static void Scrollbar(ImGuiWindow* window, bool horizontal) {
	...
	window->DrawList->AddRectFilled(bb.Min - ImVec2(1, 0), bb.Max, ImGui::GetColorU32(ImGuiCol_ScrollbarBg), window_rounding, window_rounding_corners);
	...
	if (horizontal)
		window->DrawList->AddRectFilled(ImVec2(ImLerp(bb.Min.x - 1, bb.Max.x, grab_v_norm), bb.Min.y - 2), ImVec2(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y + 2), grab_col, style.ScrollbarRounding);
	else
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x - 1, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) - 2), ImVec2(bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels + 2), grab_col, style.ScrollbarRounding);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags) {
	...
	window->DrawList->AddRectFilled(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.1f)), style.FrameRounding);
	window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), style.FrameRounding);
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg) {
	...
	window->DrawList->AddRectFilled(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), IM_COL32(0, 0, 0, 255));
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format) {
	...
	if (data_type == ImGuiDataType_Int) {
		...
		float arg1f = 0.0f;
		if (op == '+') {
			if (sscanf(buf, "%f", &arg1f) == 1) *v = (int)(arg0i + arg1f);
		}                 // Add (use "+-" to subtract)
		else if (op == '*') {
			if (sscanf(buf, "%f", &arg1f) == 1) *v = (int)(arg0i * arg1f);
		}                 // Multiply
		else if (op == '/') {
			if (sscanf(buf, "%f", &arg1f) == 1 && arg1f != 0.0f) *v = (int)(arg0i / arg1f);
		}// Divide
		else {
			if (sscanf(buf, scalar_format, &arg0i) == 1) *v = arg0i;
		}                       // Assign constant (read as integer so big values are not lossy)
		return (old_v != *v);
	}
	else if (data_type == ImGuiDataType_Float) {
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, const char* display_format, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags) {
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power) {
	...
	// Actual slider behavior + render grab
	bool value_changed = SliderBehavior(frame_bb, id, display_format, v, v_min, v_max, power, decimal_precision, ImGuiSliderFlags_Vertical);
...
bool ImGui::Checkbox(const char* label, bool* v) {
	...
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * -1, label_size.y + style.FramePadding.y * -1));
	...
	window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad - 1, pad - 1), check_bb.Max - ImVec2(pad - 1, pad - 1), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.1f)), style.FrameRounding);
	window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(33 / 255.f, 35 / 255.f, 47 / 255.f, 1.0f)), style.FrameRounding);

	if (*v) {
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark)); // Main gradient.
		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f))); // Main gradient.
	}
...
namespace ImGuiStb {
	...
	static float   STB_TEXTEDIT_GETWIDTH(STB_TEXTEDIT_STRING* obj, int line_start_idx, int char_idx) {
		ImWchar c = obj->Text[line_start_idx + char_idx]; if (c == '\n') return STB_TEXTEDIT_GETWIDTH_NEWLINE; return GImGui->Font->GetCharAdvance(c) * (GImGui->FontSize / GImGui->Font->FontSize);
	}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data) {
	...
	if (g.ActiveId == id) {
		...
		if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
			edit_state.OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINESTART : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT : STB_TEXTEDIT_K_LEFT) | k_mask);
		}
		else if (IsKeyPressedMap(ImGuiKey_RightArrow)) {
			edit_state.OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINEEND : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDRIGHT : STB_TEXTEDIT_K_RIGHT) | k_mask);
		}
		else if (IsKeyPressedMap(ImGuiKey_UpArrow) && is_multiline) {
			if (io.KeyCtrl) SetWindowScrollY(draw_window, ImMax(draw_window->Scroll.y - g.FontSize, 0.0f)); else edit_state.OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask);
		}
		else if (IsKeyPressedMap(ImGuiKey_DownArrow) && is_multiline) {
			if (io.KeyCtrl) SetWindowScrollY(draw_window, ImMin(draw_window->Scroll.y + g.FontSize, GetScrollMaxY())); else edit_state.OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask);
		}
			...
			ClearActiveID(); cancel_edit = true;
		}
			...
			for (const ImWchar* s = text_begin; *s != 0; s++)
				if (*s == '\n') {
					...
					if (searches_result_line_number[0] == -1 && s >= searches_input_ptr[0]) {
						searches_result_line_number[0] = line_count; if (--searches_remaining <= 0) break;
					}
					if (searches_result_line_number[1] == -1 && s >= searches_input_ptr[1]) {
						searches_result_line_number[1] = line_count; if (--searches_remaining <= 0) break;
					}
				}
			line_count++;
...
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items) {
	...
	bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, (height_in_items == -1 ? items_count : height_in_items));
	return value_changed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	auto arrow = [window](int x, int y, float clr = 0.59) {
		for (auto i = 5; i >= 2; --i) {
			...
			window->DrawList->AddLine(ImVec2(x + offset, y + offset), ImVec2(x + offset + nigga(i - offset, 0, 5), y + offset), GetColorU32(ImVec4(clr, clr, clr, 1)));
		}
	};

	arrow(frame_bb.Max.x - 10, frame_bb.Max.y - 11, 0.f);
	...
	if ((popup_y2 - popup_y1) < ImMin(popup_size.y, frame_bb.Min.y - style.DisplaySafeAreaPadding.y))
	{
		...
		SetNextWindowPos(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImGuiCond_Always);
	}
	else
	{
		// Position our combo below
		SetNextWindowPos(ImVec2(frame_bb.Min.x, frame_bb.Max.y + 3), ImGuiCond_Always);
	}
	SetNextWindowSize(ImVec2(popup_size.x, popup_size.y), ImGuiCond_Always);
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items) {
	...
	if (*current_item >= 0 && *current_item < items_count) {
		...
		if (items_getter(data, *current_item, &item_text))
			RenderTextClipped(frame_bb.Min + ImVec2(6, 1), value_bb.Max, item_text, NULL, NULL, ImVec2(0.0f, 0.0f));
	}
	...
	bool value_changed = false;
	if (IsPopupOpen(id)) {
		// Size default to hold ~**5** items
		if (height_in_items < 0)
			height_in_items = 5;

		float popup_height = -8.0f + (label_size.y + style.ItemSpacing.y) * ImMin(items_count, height_in_items) + (style.FramePadding.y * 3);
		float popup_y1 = frame_bb.Max.y;
		...
		SetNextWindowSize(popup_rect.GetSize() + ImVec2(2, 0));
...
bool ImGui::SelectableMeme(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	float spacing_L = (float)(int)(0);
	float spacing_U = (float)(int)(0);
	...
	// Render
	if (selected) {
		const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : ImGuiCol_Header);
	...
	RenderTextClipped(bb.Min + ImVec2(4, 2), bb_with_spacing.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f));
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg) {
	...
	ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 5), window->DC.CursorPos + frame_size);
	ImRect bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	window->DC.LastItemRect = bb;

	SetCursorPosX(GetCursorPosX() + 17);
	BeginGroup();
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Min.x + 17, frame_bb.Min.y - 20), label);
...
bool ImGui::ListBox(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items) {
	...
	while (clipper.Step())
		for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
			...
			if (SelectableMeme(item_text, item_selected, 0, ImVec2(0, 0))) {
				*current_item = i;
				value_changed = true;
			}
			PopID();
	...
	if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF) {
		...
		for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++) {
			...
			for (float x = p_min.x + grid_off.x + (yi & 1) * grid_step; x < p_max.x; x += grid_step * 2.0f) {
				...
				if (y1 <= p_min.y) {
					if (x1 <= p_min.x) rounding_corners_flags_cell |= ImGuiCorner_TopLeft;    if (x2 >= p_max.x) rounding_corners_flags_cell |= ImGuiCorner_TopRight;
				}
				if (y2 >= p_max.y) {
					if (x1 <= p_min.x) rounding_corners_flags_cell |= ImGuiCorner_BottomLeft; if (x2 >= p_max.x) rounding_corners_flags_cell |= ImGuiCorner_BottomRight;
				}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size) {
	...
	window->DrawList->AddRectFilled(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), GetColorU32(ImGuiCol_Border), 0); // Color button are often in need of some sort of border
	...
	if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f) {
		...
		window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(mid_x, bb.Max.y), GetColorU32(ImVec4(0, 0, 0, 0.078f)), GetColorU32(ImVec4(0, 0, 0, 0.078f)), GetColorU32(ImVec4(0, 0, 0, 0.45f)), GetColorU32(ImVec4(0, 0, 0, 0.45f)));
	}
	else {
		...
		window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImVec4(0, 0, 0, 0.078f)), GetColorU32(ImVec4(0, 0, 0, 0.078f)), GetColorU32(ImVec4(0, 0, 0, 0.45f)), GetColorU32(ImVec4(0, 0, 0, 0.45f)));
	}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags) {
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
		...
		if (BeginPopupEx(g.CurrentWindow->GetID("picker"), ImGuiWindowFlags_NoMove | ImGuiWindowFlags_ShowBorders)) {
			picker_active = true;
	...
	ImGui::GetStyle().WindowPadding = ImVec2(16, 18);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) {
	...
	// Setup
	bool alpha_bar = true/*, because fuck you.*/;
	ImVec2 picker_pos = window->DC.CursorPos;
	float bars_width = 14; // Arbitrary smallish width of Hue/Alpha picking bars
	float sv_picker_size = 160; // ImMax( bars_width * 1, CalcItemWidth( ) - ( alpha_bar ? 2 : 1 ) * ( bars_width + style.ItemInnerSpacing.x ) ); // Saturation/Value picking box
	...
	if (flags & ImGuiColorEditFlags_PickerHueBar) {
		...
		draw_list->AddRectFilled(ImVec2(bar0_pos_x - 1, picker_pos.y - 1), ImVec2(bar0_pos_x + bars_width + 1, picker_pos.y + sv_picker_size + 1), IM_COL32(0, 0, 0, 255));
		...
		draw_list->AddRectFilled(ImVec2(bar0_pos_x + 1, bar0_line_y + 1), ImVec2(bar0_pos_x + bars_width - 1, bar0_line_y + 3), hue_color32);
	}
	...
	if (alpha_bar) {
		...
		draw_list->AddRect(ImVec2(bar1_pos_x, bar1_line_y), ImVec2(bar1_pos_x + bars_width, bar1_line_y + 4), IM_COL32_BLACK);
		draw_list->AddRectFilled(ImVec2(bar1_pos_x + 1, bar1_line_y + 1), ImVec2(bar1_pos_x + bars_width - 1, bar1_line_y + 3), IM_COL32(col[0], col[1], col[2], col[3]));
	}
